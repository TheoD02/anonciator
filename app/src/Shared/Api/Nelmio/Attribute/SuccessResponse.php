...
class SuccessResponse extends Response
{
    public function __construct(
        ...
        $model = new Model(type: $dataFqcn, serializationContext: $serializationContext);
        if ($paginated) {
            $dataProperty->type = 'array';
            $dataProperty->items = new Items(ref: $model, description: $description);
        } else {
            $dataProperty->type = 'object';
            $dataProperty->ref = $model;
        }
